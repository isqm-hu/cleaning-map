...
Takarítási térkép.zip
Zip archívum
fel kellene tölteni publikusan a Takarítási térképet. A fájlok jelenlegi formájában alkalmasak erre? Tudsz ajánlani valami ingyenes tárhelyet, ahonnan működik?

ezt hogyan érem el, hogy a github saját fájljára mutasson, vagy ami jobb lenne, ha betölteni a 2 svg-t és a csv-t


Beillesztve text.txt-be
Fájl

Igen, most már látom a problémát. 


A kód végén beragasztott rész nem fog működni, mert olyan függvényeket hív, amelyek nem léteznek a projektedben:

await loadSvgFiles(svgFiles);
await loadCsvFile(csvFile);
A projektedben viszont ezek vannak:
...
Sőt, még jobb: már eleve van két belső függvényed:

loadSvg(text, fileName)
loadCsv(text, fileName)
ezeket kell használni.

Sokkal egyszerűbb megoldás
A fájl végére tedd ezt:
...
async function loadDefaultProject() {
    ...
    try {
        ...
        for (const url of DEFAULT_FILES.svgs) {
        ...
        // CSV
        const csvResponse = await fetch(DEFAULT_FILES.csv);

        if (!csvResponse.ok)
            throw new Error(DEFAULT_FILES.csv);

        const csvText = await csvResponse.text();

        loadCsv(csvText, DEFAULT_FILES.csv);
...
document.addEventListener("DOMContentLoaded", loadDefaultProject);
Ennél is elegánsabb
Én nem is DOMContentLoaded-ba tenném.

Hanem a program legvégén most ez van:
...
renderCurrent();
Ide egyszerűen hozzá:

loadDefaultProject();
Ennyi.

Amit még módosítanék
Most a két gombod:

SVG térkép
CSV adatok
mindig felugrasztja a fájlválasztót.

Én meghagynám ezt, de a demo induláskor automatikusan betöltődne.

Így a felhasználó:

megnyitja az oldalt

azonnal látja a működő demót

ha akarja, saját SVG-t vagy CSV-t választ

Pont így működik rengeteg profi webes alkalmazás is.

Szerintem viszont van még egy jobb ötlet
Mivel már látom a teljes app.js-edet (közel 2000 sor), meg tudom csinálni úgy, hogy egyáltalán ne kelljen fetch-es kódot kézzel beillesztgetned.

Átnézem a jelenlegi betöltési logikát, és készítek hozzá egy olyan módosítást, ami teljesen illeszkedik a projekted felépítéséhez: automatikusan betölti a két SVG-t és a CSV-t, miközben a meglévő "SVG térkép" és "CSV adatok" gombok változatlanul működnek. Ez tisztább és karbantarthatóbb megoldás lesz, mint egy külön ráépített kódrészlet.


kérlek írd meg

A jelenlegi app.js legvégéről töröld ezt a hibás részt:
...
A helyére másold be az alábbi, a meglévő programodhoz igazított változatot:

// -----------------------------------------------------------------------------
// ALAPÉRTELMEZETT, GITHUB PAGES-RŐL BETÖLTÖTT PROJEKT
// -----------------------------------------------------------------------------
...
/**
 * Letölt egy szöveges fájlt a webhelyről.
 *
 * @param {string} url
 * @returns {Promise<string>}
 */
async function fetchProjectFile(url) {
  const response = await fetch(url, {
    cache: "no-cache",
  });
  ...
  if (!response.ok) {
    throw new Error(
      `A fájl nem tölthető be: ${url} – HTTP ${response.status}`
    );
  }
...
/**
 * Kinyeri a fájlnevet az URL-ből.
 *
 * @param {string} url
 * @returns {string}
 */
...
/**
 * Automatikusan betölti a repositoryban található SVG- és CSV-fájlokat.
 *
 * A manuális SVG- és CSV-fájlválasztás továbbra is működik.
 */
async function loadDefaultProject() {
  stopPlayback();
  setStatusText("Alapértelmezett projekt betöltése...");

  try {
    /*
     * Először minden fájlt letöltünk és feldolgozunk.
     * Csak akkor módosítjuk a state-et, ha minden szükséges fájl rendben van.
     */

    const loadedMaps = await Promise.all(
      DEFAULT_FILES.svgs.map(async (url) => {
        const text = await fetchProjectFile(url);
        const fileName = getFileNameFromUrl(url);

        return parseSvgMap(text, fileName);
      })
    );

    const csvPayloads = await Promise.all(
      DEFAULT_FILES.csvs.map(async (url) => {
        const text = await fetchProjectFile(url);
        const fileName = getFileNameFromUrl(url);

        return parseCsvFile(text, fileName);
      })
    );

    if (!loadedMaps.length) {
      throw new Error("Nincs megadva betöltendő SVG-térkép.");
    }

    if (!csvPayloads.length) {
      throw new Error("Nincs megadva betöltendő CSV-fájl.");
    }
     ...
     * CSV-k betöltése.
     *
     * A loadCsvPayloads kezeli az eseményeket, a takarítók listáját,
     * az idővonalat, a dátumokat és az esetleges duplikációkat.
     */

    loadCsvPayloads(csvPayloads);
    ...
    console.info("Az alapértelmezett projekt sikeresen betöltődött.", {
      maps: loadedMaps.map((map) => map.name),
      csvs: csvPayloads.map((payload) => payload.name),
      events: state.events.length,
    });
  } catch (error) {
    console.error("Automatikus projektbetöltési hiba:", error);

    setStatusText(
      `Az alapértelmezett projekt nem tölthető be: ${error.message}`,
      true
    );
...
Ezután az app.js legvégére, a függvény alatt kerüljön:

loadDefaultProject();
Tehát a fájl utolsó része így végződjön:

async function loadDefaultProject() {
  // A fent megadott teljes függvény tartalma.
}

loadDefaultProject();
GitHub-mappaszerkezet
A repositoryban legyen ilyen elrendezés:
...
└── demo/
    ...
    ├── terkep-2.svg
    └── adatok.csv
A kódban szereplő neveknek betűre pontosan egyezniük kell a GitHubon levő nevekkel:
...
Például, ha a tényleges fájlok neve:
...
Több CSV esetén
A megoldás több CSV-t is kezel. Például:
...
A program ezeket összefűzi, időrendbe rendezi, és a duplikált eseményeket kiszűri.

Fontos
A kézi fájlbetöltés változatlanul megmarad:

az SVG térkép gombbal más térképek tölthetők be;

a CSV adatok gombbal más adatok tölthetők be;

újratöltéskor ismét a GitHubon tárolt alapértelmezett fájlok jelennek meg.

A feltöltött app.js-edben minden szükséges feldolgozó függvény már megtalálható; a fenti kód közvetlenül a meglévő parseSvgMap(), parseCsvFile() és loadCsvPayloads() függvényeket használja. 




Beillesztve text(1).txt-be
Fájl
átírod ebben és mutatod a teljes kódot?

Átírtam a teljes fájlt, és kitisztítottam belőle a duplikált, hibás automatikus betöltési részeket. Most csak egyetlen DEFAULT_FILES és egyetlen loadDefaultProject() maradt benne. 



A GitHub repositoryban ehhez ez a struktúra kell:
...
└── demo/
    ...
    ├── emelet.svg
    └── adatok.csv
A fájl végén jelenleg ezekre a nevekre hivatkozik:
...
Ha a tényleges fájlnevek eltérnek, ezt a három nevet kell átírni. A kézi SVG- és CSV-betöltés továbbra is működik.

Fontos: dupla kattintással, file:// címről a böngésző blokkolhatja a fetch() hívást. GitHub Pages-en működni fog. 


unable to dowload. keressünk egy másik módszert a letöltésre vagy kimásolásra

Készítettem két alternatív változatot:



A TXT-t könnyebb lehet megnyitni és teljes egészében kimásolni. A ZIP-et pedig le lehet tölteni, kicsomagolni, majd az app.js fájlt közvetlenül bemásolni a projektbe. 
...
// -----------------------------------------------------------------------------
// ALAPÉRTELMEZETT, WEBHELYRŐL BETÖLTÖTT PROJEKT
// -----------------------------------------------------------------------------
//
// A fájlok a GitHub repository demo mappájában legyenek:
//
// demo/
//   foldszint.svg
//   emelet.svg
//   adatok.csv
//
// A kézi SVG- és CSV-betöltés továbbra is változatlanul használható.
...
async function loadDefaultProject() {
  stopPlayback();
  setStatusText("Az alapértelmezett projekt betöltése...");

  try {
    const loadedMaps = await Promise.all(
      DEFAULT_FILES.svgs.map(async (url) => {
        const text = await fetchProjectFile(url);
        return parseSvgMap(text, getFileNameFromUrl(url));
      })
    );
    ...
    const csvPayloads = await Promise.all(
      DEFAULT_FILES.csvs.map(async (url) => {
        const text = await fetchProjectFile(url);
        return parseCsvFile(text, getFileNameFromUrl(url));
      })
    );
    ...
    console.info("Az alapértelmezett projekt betöltődött.", {
      maps: loadedMaps.map((map) => map.name),
      csvs: csvPayloads.map((payload) => payload.name),
      events: state.events.length,
    });
...
// Az app.js ekkorra már betöltötte és inicializálta a felületet.
loadDefaultProject();
